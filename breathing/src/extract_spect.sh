...
clip_below_values="-60"

# Parser for the data set
parser="audeep.backend.parsers.compare20_breathing.Compare20BreathingParser"
...
for clip_below_value in ${clip_below_values}; do
    ...
    if [ ! -f ${spectrogram_file} ]; then
        ...
        echo
        audeep preprocess${verbose_option} --parser ${parser} --basedir ${audio_base} --output ${spectrogram_file} --window-width ${window_width} --window-overlap ${window_overlap}  --clip-below ${clip_below_value} --mel-spectrum ${mel_bands}
    fi
done
