...
import end2end.data_read.data_spect_select as data_provider
    ...
    with g.as_default():
        with tf.Session(config=config) as sess:
            ...
            dataset_train = data_provider.get_partition(tf_records_folder,
                                                        ...
                                                        buffer_size=(train_steps_per_epoch + 1), frame_size=configuration.frame_size)
            dataset_devel = data_provider.get_partition(tf_records_folder,
                                                        ...
                                                        buffer_size=(devel_steps_per_epoch + 1) // 4, frame_size=configuration.frame_size)
            dataset_test = data_provider.get_partition(tf_records_folder,
                                                       is_training=False,
                                                       partition_name="test",
                                                       batch_size=configuration.test_batch_size,
                                                       seq_length=configuration.full_seq_length,
                                                       buffer_size=(test_steps_per_epoch + 1) // 4, frame_size=configuration.frame_size)
            ...
            print("Dataset output shapes: ", dataset_train.output_shapes)
            ...
            audio_train = tf.placeholder(tf.float32, (None, configuration.train_seq_length, configuration.frame_size))
            spect_train = tf.placeholder(tf.float32, (None, configuration.train_seq_length, 128))
            ...
            audio_test = tf.placeholder(tf.float32, (None, configuration.full_seq_length, configuration.frame_size))
            spect_test = tf.placeholder(tf.float32, (None, configuration.full_seq_length, 128))
            ...
            with tf.variable_scope("Model", reuse=True):
                pred_test = core.get_cnn_lstm_model(audio=audio_test, spect=spect_test,
                                                    batch_size=batch_size_tensor,
                                                    num_layers=configuration.num_layers,
                                                    hidden_units=configuration.hidden_units,
                                                    number_of_outputs=number_of_targets)
            ...
            loss = core.loss_function(pred_upper_belt=single_pred_upper_belt_train,
                                      true_upper_belt=upper_belt_train, alpha=configuration.alpha)
            ...
            for ee, epoch in enumerate(range(configuration.num_epochs)):
                ...
                if (ee) % configuration.val_every_n_epoch == 0:
                    ...
                    if best_performance < devel_measures["pearson"]:
                        ...
                        #core.make_label_file(path=output_folder + "/dev_pred.csv", data=devel_items.upper_belt.pred.reshape((16, 6000)))
                        best_dev = devel_items.upper_belt.pred.reshape((16, 6000))
                        #core.make_label_file(path=output_folder + "/dev_true.csv", data=devel_items.upper_belt.true.reshape((16, 6000)))
            ...
            run_epoch = core.RunEpoch(sess=sess,
                                      ...
                                      pred=pred_test,
                                      input_feed_dict={batch_size_tensor: "batch_size",
                                                       audio_test: "audio", spect_test: "spect"},
                                      targets=targets)

            test_items, test_subject_to_id = run_epoch.run_epoch()

            core.make_label_file(path=output_folder + "/test_pred.csv",
                                 data=test_items.upper_belt.pred.reshape((16, 6000)))
            print("Best devel pcc:", best_performance)
            return best_dev,test_items.upper_belt.pred.reshape((16, 6000)) # best_dev
