...
def get_cnn_lstm_model(audio, spect,
                       batch_size,
                       num_layers,
                       hidden_units,
                       number_of_outputs):
    ...
    print('Input shape: ', audio.get_shape().as_list(), spect.get_shape().as_list())
    ...
    net_audio_c = tf.reshape(net, [batch_size, seq_length, 256])       
    spect_input = spect
  
    net = tf.layers.conv1d(spect_input, 64, 8, padding='same', activation=tf.nn.relu)

    net_spect_c = tf.layers.conv1d(net, 64, 6, padding='same', activation=tf.nn.relu)
    
    net = tf.concat([net_audio_c,net_spect_c],axis=2) # 

    _, seq_length, num_features = net.get_shape().as_list()
    print('CNN out shape: ',  net.get_shape().as_list())
    def _get_cell(l_no):
...
def loss_function(pred_upper_belt, true_upper_belt, alpha=0.0):
    true_upper_belt = tf.reshape(true_upper_belt, (-1, 1, 1, 1))

    support = tf.ones_like(true_upper_belt)

    return weighted_pearson_cc(pred_upper_belt, support, true_upper_belt)+alpha*mse(pred_upper_belt, true_upper_belt)
    #for MSE use the line below
    #return mse(pred_upper_belt, true_upper_belt)

def mse(pred, true):
    return tf.reduce_mean(tf.squared_difference(pred, true))
...
class RunEpoch:
    ...
    def run_epoch(self):
        ...
        for step in range(self.steps_per_epoch):
            ...
            feed_dict = replace_dict_value(feed_dict, "spect", spect)
    ...
    def run_epoch(self):
        ...
        for step in range(self.steps_per_epoch):
            ...
            if self.grads is not None:
                out_tf.append(self.grads)
                grads_index = len(out_tf) - 1
            ...
            if self.loss is not None:
                stored_variables["loss"] = out_np[loss_index]
            if self.grads is not None:
                stored_variables["grads"] = out_np[grads_index][0]
                print(stored_variables["grads"].shape)
        for target in self.targets:
            if self.partition in ["train", "devel"]:
                stored_variables[target]["true"] = stored_variables[target]["true"][:batch_size_sum, :]
            stored_variables[target]["pred"] = stored_variables[target]["pred"][:batch_size_sum, :]
