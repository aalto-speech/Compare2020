...
import os
for i in range(10):
    configuration["output_folder"] = OUTPUT_FOLDER+"_multi_"+str(i)
    ...
    np.save('test_'+str(i)+'_spect_loss_fselect_'+str(configuration["alpha"]), test_dat)
    np.save('dev_'+str(i)+'_spect_loss_fselect_'+str(configuration["alpha"]), dev_pred_s)
...
#core.make_label_file(path="ComParE_Breathin_e2e_preemph_raw_"+str(configuration["alpha"])+"_pred.csv", data=test_pred)
